    ...
    internal static class FacilityAdjustments
    {
        ...
        public static void Apply()
        {
            ...
            foreach (ExperienceFacilityComponentDef efcDef in experienceFacilityComponentDefs)
            {
                ...
                {
                    efcDef.ExperiencePerUser = Settings.TrainingFacilityBaseExperienceAmount;
                    currentExperienceFacilityExperienceOutput = efcDef.ExperiencePerUser;

                    efcDef.SkillPointsPerDay = Settings.TrainingFacilityBaseSkillPointsAmount;
                }
            ...
            {
                vsfDef.AircraftHealAmount = Settings.VehicleBayAircraftHealAmount;
                vsfDef.VehicleHealAmount = Settings.VehicleBayVehicleHealAmount;
            ...
            foreach (ResourceGeneratorFacilityComponentDef rgfDef in resourceGeneratorFacilityComponentDefs)
            {
                if (rgfDef.name.Contains("FabricationPlant"))
                {
                    ...
                    ResourceUnit supplies = new ResourceUnit(ResourceType.Production, Settings.FabricationPlantGenerateProductionAmount);
                ...
                {
                    ResourcePack resources = rgfDef.BaseResourcesOutput;
                    ResourceUnit supplies = new ResourceUnit(ResourceType.Research, Settings.ResearchLabGenerateResearchAmount);
                    resources.Set(supplies);
                ...
                {
                    ResourcePack resources = rgfDef.BaseResourcesOutput;
                    ResourceUnit supplies = new ResourceUnit(ResourceType.Supplies, Settings.FoodProductionGenerateSuppliesAmount);
                    resources.Set(supplies);

                    currentFoodProductionFacilitySuppliesOutput = Settings.FoodProductionGenerateSuppliesAmount;
                }
                else if (rgfDef.name.Contains("BionicsLab"))
                {
                    ResourcePack resources = rgfDef.BaseResourcesOutput;
                    ResourceUnit supplies = new ResourceUnit(ResourceType.Research, Settings.BionicsLabGenerateResearchAmount);
                    resources.Set(supplies);
                }
                else if (rgfDef.name.Contains("MutationLab"))
                {
                    ResourcePack resources = rgfDef.BaseResourcesOutput;
                    ResourceUnit supplies = new ResourceUnit(ResourceType.Mutagen, Settings.MutationLabGenerateMutagenAmount);
                    resources.Set(supplies);
                }
            ...
            if(Settings.TrainingFacilitiesGenerateSkillpoints && Settings.TrainingFacilityBaseSkillPointsAmount > 0)
            {
        ...
        public static void Postfix_ResourceGeneratorFacilityComponent_UpdateOutput(ResourceGeneratorFacilityComponent __instance)
        {
            try
            {
                ...
                // All factions
                if (__instance.Def.name.Contains("FabricationPlant") && Settings.FabricationPlantGenerateMaterialsAmount > 0)
                {
                    float value = Settings.FabricationPlantGenerateMaterialsAmount / Settings.GenerateResourcesBaseDivisor;
                    ResourceUnit materials = new ResourceUnit(ResourceType.Materials, value);
                    __instance.ResourceOutput.AddUnique(materials);
                }
                else if (__instance.Def.name.Contains("ResearchLab") && Settings.ResearchLabGenerateTechAmount > 0)
                {
                    float value = Settings.ResearchLabGenerateTechAmount / Settings.GenerateResourcesBaseDivisor;
                    ResourceUnit tech = new ResourceUnit(ResourceType.Tech, value);
        ...
        public static void Postfix_UIFacilityTooltip_Show(UIFacilityTooltip __instance, PhoenixFacilityDef facility, GeoPhoenixBase currentBase)
        {
            try
            {
                ...
                else if (facility.name.Contains("TrainingFacility"))
                {
                    ...
                    if(Settings.TrainingFacilitiesGenerateSkillpoints && Settings.TrainingFacilityBaseSkillPointsAmount > 0)
                    {
                        string pluralizeSP = Settings.TrainingFacilityBaseSkillPointsAmount > 1 ? "skillpoints" : "skillpoint";
                        s2 = $"Contributes {Settings.TrainingFacilityBaseSkillPointsAmount} {pluralizeSP} to the global pool every day."; 
                    }
                ...
                else if (facility.name.Contains("FabricationPlant") && Settings.FabricationPlantGenerateMaterialsAmount > 0)
                {
                    string org = __instance.Description.text;
                    string add = $"Every plant generates {Settings.FabricationPlantGenerateMaterialsAmount} material per hour.";
                    __instance.Description.text = $"{org}\n{add}";
                }
                else if (facility.name.Contains("ResearchLab") && Settings.ResearchLabGenerateTechAmount > 0)
                {
                    string org = __instance.Description.text;
                    string add = $"Every lab generates {Settings.ResearchLabGenerateTechAmount} tech per hour.";
                    __instance.Description.text = $"{org}\n{add}";
                }
        ...
        public static void Postfix_UIFacilityInfoPopup_Show(UIFacilityInfoPopup __instance, GeoPhoenixFacility facility)
        {
            try
            {
                ...
                else if (facility.Def.name.Contains("FabricationPlant") && Settings.FabricationPlantGenerateMaterialsAmount > 0)
                {
                    string org = __instance.Description.text;
                    string add = $"Every plant generates {Settings.FabricationPlantGenerateMaterialsAmount} material per hour.";
                    __instance.Description.text = $"{org}\n{add}";
                }
                else if (facility.Def.name.Contains("ResearchLab") && Settings.ResearchLabGenerateTechAmount > 0)
                {
                    string org = __instance.Description.text;
                    string add = $"Every lab generates {Settings.ResearchLabGenerateTechAmount} tech per hour.";
                    __instance.Description.text = $"{org}\n{add}";
